...
| 1.10 | **Elements + crits** (audit §8): Elements component is populated but never read; no critical hits. Both plug into the waiting `TweakDamageEvent` stage (Doused's fire-vulnerability upgrade rides along). Also wires the element-tagged GearTraits from 1.9b (TweakDamage gains an element field; Rattan Jacket's fire weakness goes live). | queued |
