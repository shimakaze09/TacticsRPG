...
| 1.5c | **AI act-then-move**: the tactical AI should weigh both turn orderings — attack from the current tile *then* retreat (hit-and-run; score = attack value + safety of the retreat tile) versus today's move-then-attack. Needs: `PlanOfAttack` ordering flag + post-act move location; `CommandSelectionState.ComputerTurn` executes the chosen order (the turn economy already permits act-before-move — humans can do it). Kiting priority for ranged/caster jobs. Easy AI stays simple. | queued |
| 1.5d | **AI threat map** (foundation): per-tile expected damage from enemies that could reach+hit it next turn. Consumed by 1.5c's retreat leg, 1.5e, 1.5g, and safer end-tile tie-breaks. | queued |
| 1.5e | **AI self-preservation**: below ~30% HP with no kill available, retreat toward an allied healer that has MP and range (else safest tile); healers hold position outside enemy reach by default. | queued |
